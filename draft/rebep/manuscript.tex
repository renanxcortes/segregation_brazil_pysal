...
los sectores censales del Censo 2022, y la poblaci\'on \emph{preta} y \emph{parda}
sumada frente a la poblaci\'on total, estudiamos los 319 municipios con m\'as de
100 mil habitantes. Para cada uno calculamos nueve \'indices que abarcan las
cinco dimensiones de Massey y Denton --- uniformidad, exposici\'on,
concentraci\'on, centralizaci\'on y agrupamiento --- con el m\'odulo
\texttt{segregation} de PySAL, mediante estimaci\'on puntual, sin inferencia. La
segregaci\'on de uniformidad es moderada, con un \'indice de disimilitud mediano
cercano a 0,20, y se concentra en el Sur y el Sudeste, lo que confirma y
ampl\'ia an\'alisis nacionales previos con datos de 2010. Los cuatro \'indices de
...
de Kendall de cerca de $-0{,}26$. Dos a tres dimensiones independientes de
...
The complete pipeline is public. The \texttt{segbr} Python package performs data loading, city assembly and the computation of the nine measures; driver scripts build the city universe and run the nationwide computation; and further scripts generate every figure and table. A pinned environment specification and a manifest of the IBGE source files accompany the code. Every figure and table in this paper regenerates from the committed city-level results table with a single command, so the analysis can be reproduced end to end or inspected at any intermediate stage. The repository is available at \url{REPOSITORY-URL}.
